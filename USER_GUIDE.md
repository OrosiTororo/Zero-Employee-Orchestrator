...
**サブスクリプションモード**は、API キーを一切設定しなくても AI が使えるモードです。g4f（gpt4free）ライブラリを使って、各社の **無料 Web サービスを経由して利用**します。

> **注意**: このモードは外部 Web サービスの無料版に依存するため、**レート制限や一時的な利用不可が頻繁に発生**します。
> あくまで試用・テスト目的であり、安定した利用には方法 A（Gemini 無料 API）または方法 B（Ollama）を推奨します。
...
| `g4f/Copilot` | Microsoft Copilot | 不要 |
| `g4f/OpenaiChat` | ChatGPT Web | 不要 |
| `g4f/Claude` | Claude Haiku | 不要 |
...
各プロバイダーの API キーを `apps/api/.env` に設定します:
