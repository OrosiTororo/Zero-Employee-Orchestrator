...
AI タスクを実行するには、**LLM プロバイダーへの接続が必要**です。次のセクションで選択肢を詳しく説明します。

---

## 4. LLM（AI）の接続方法

### ⚠️ よくある誤解：サブスクリプションと API は別物です

ChatGPT Plus（月 $20）・Gemini Advanced・Claude Pro などのサブスクリプションは **Web サイトやスマートフォンアプリ向けのサービス** です。これらのサブスクリプションに加入しても、Zero-Employee Orchestrator からプログラムで直接 AI を呼び出す「API アクセス」は得られません。

**API アクセスは別の料金体系**（多くの場合、使用したトークン数に応じた従量課金）で提供されています。

### 選択肢の比較

| 方法 | 費用 | API キー | インターネット |
|------|------|----------|--------------|
| Google Gemini 無料枠 | **無料**（上限あり） | 必要（無料取得） | 必要 |
| Ollama（ローカル） | **完全無料** | 不要 | 不要 |
| OpenRouter | 従量課金 | 必要 | 必要 |
| OpenAI | 従量課金 | 必要 | 必要 |
| Anthropic | 従量課金 | 必要 | 必要 |

---

### 方法 A: Google Gemini 無料 API（コストゼロで始める場合に推奨）
...
### Q: ChatGPT Plus に加入していればサブスクリプション料金だけで使えますか？

**A: いいえ。** ChatGPT Plus・Gemini Advanced・Claude Pro などのサブスクリプションは **Web サイト/アプリ向け** のサービスです。Zero-Employee Orchestrator のようなプログラムから API 経由で直接呼び出すことはできません。

プログラム用の API アクセスは OpenAI Platform・Google AI Studio・Anthropic Console などで**別途取得・課金**が必要です。

**費用をかけずに始めたい場合:**
- Google Gemini の無料 API キーを取得する（[方法 A](#方法-a-google-gemini-無料-apiコストゼロで始める場合に推奨) 参照）
- Ollama でローカル LLM を実行する（[方法 B](#方法-b-ollamaローカル-llm完全無料) 参照）
