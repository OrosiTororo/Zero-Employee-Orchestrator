...
**既存のAIエージェントのような単発タスクの自動化やマルチステップの遂行、長期的意思決定 /自律計画修正、自律的環境センサー + イベントトリガーだけでなく、マルチエージェント協調・統制や高度な自己改善・反省ループ、自然言語のみで業務を遂行するためのSkillsを自動で組み合わせ・構築し、ユーザーの任意で共有・公開が可能（個人情報や機密は共有せずプライバシーが保たれるようにする）である。**
...
| 6 | **レート制限の実装と情報・現状把握のためのセントラルリポジトリ** | 最高 | OpenRouter API のレート制限に対するバックプレッシャー制御 と作業の複雑さや種類、量の予想からAPI料金を抑える工夫を行うために、最新のモデル更新や各AIモデルの特性や料金をAIが把握と制限ができるようにする。特に自動情報更新はAPI料金が掛かるので、抑える工夫が必要である。そのため更新する情報と、把握に必要な現状が重複しないように、共通の置き場所（セントラルリポジトリ）を作っておくこと。|
...
注）この改善案反映後は機能一覧を最新のものに更新すること。さらに改善案がある場合は再度このファイルに追加すること。**単発タスクの自動化やマルチステップの遂行、長期的意思決定 /自律計画修正、自律的環境センサー + イベントトリガー、マルチエージェント協調・統制や高度な自己改善・反省ループ、自然言語のみで業務を遂行するためのSkillsを自動で組み合わせ・構築し、ユーザーの任意で共有・公開が可能（個人情報や機密は共有せずプライバシーが保たれるようにする）であることを確認すること。**構築されるSkillsは攻撃的・有害であったり、ガイドラインに反するものは作成・共有・公開を許可しないこと。また、そのような方向性にならないように調整すること。
---
...
このシステムはAIなどの技術の進歩に柔軟に対応できるようにしたい。しかし使いやすさを保つためにも、ある程度形や機能が固まったらシステム自体のアップデートは最小限に、最新技術をSkillsだけで追加できるようにしたい。このシステムのSkills作成における組み合わせ・構築機能によって革新的な技術や業務遂行が生まれるのはロマンを感じる。

### Skillsについて

ZPCOSにおいて、ユーザーが最初にインストールした段階ではSkillsは最低限しか無い状態であり、ユーザーがチャットでAIと対話し合い、AIに行わせたい業務を遂行するためのSkillsを組み合わせたり、新しく構築する。また、YouTube Skillsのように公式や他のユーザーが作成した業務遂行Skillsを追加することもできるプラットフォームがあり、正にAI版VCodeのように拡張機能のような形でカスタマイズできるのだ。最初から詰め込むのではなく、必要なものを必要な分だけ取り入れることで、どんな環境にも適応、応用できる自由度の高い拡張的で柔軟なシステムとなる。
その為、特定の作業手順や専門知識（プロンプト、スクリプト、リソース）をパッケージ化し、AIが必要に応じて自動的に読み込んで実行できる「追加能力」機能であるSkillsの活用例の広さについて、以下に記す。
