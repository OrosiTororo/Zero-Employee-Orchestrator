...
### ChatGPT / Claude / Gemini（チャット型 AI）との違い

| 観点 | 従来のチャット AI | Zero-Employee Orchestrator |
|------|-----------------|---------------------------|
| **記憶** | 会話が終わると忘れる | 経験・パターン・失敗を永続記憶 |
| **タスク継続性** | 毎回ゼロから | チケットで状態を永続管理 |
| **複数 AI の協力** | 1対1の会話のみ | 役割分担した AI チームが並列実行 |
| **品質保証** | ユーザーが目視確認 | Judge Layer が自動で二段階検証 |
| **危険操作のガード** | なし | 承認フローで必ずブロック |
| **監査ログ** | なし | 全操作を追跡・記録 |
| **失敗時の対応** | 手動でやり直し | Self-Healing DAG が自動再計画 |
...
AI ツールの導入障壁の一つが「API キーの取得・設定」です。

Zero-Employee Orchestrator の**サブスクリプションモード**なら、設定ファイルに1行追加するだけで始められます:

```env
DEFAULT_EXECUTION_MODE=subscription
```

Gemini・ChatGPT・Copilot・Claude などの無料 Web サービスを自動的に利用します。API キーの取得も支払い情報の入力も不要です。

本格的に使い始めたら、Google Gemini の無料 API キー（クレジットカード不要）や Ollama によるローカル実行に移行できます。
...
Plugin  — 外部サービス連携（例: Slack、Google Drive、Salesforce）
Extension — UI・動作のカスタマイズ
```
