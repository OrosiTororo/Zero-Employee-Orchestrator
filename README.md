...
| **壁打ち（ブレインストーミング）** | AI相談役と壁打ち。複数モデル同時比較で回答を見比べ可能 |
| **マルチモデル比較** | GPT / Gemini / Claude 等に同一入力を送り、回答を並べて比較 |
| **会話記憶** | ユーザーとAI組織の全会話を永続保管。過去の発言を参照可能 |
| **役割別モデル設定** | エージェントの役割ごとに使用するAIモデルをユーザーが自由に設定 |
| **正確な文字数カウント** | 日本語・中国語を含むUnicode対応の正確な文字数カウント |
| **動的エージェント管理** | 秘書・相談役等のエージェントをユーザーが追加・削除・役割変更可能 |
| **自然言語組織管理** | 「相談役を追加して」等の自然言語でAI組織を管理 |
...
[Releases ページ](https://github.com/OrosiTororo/Zero-Employee-Orchestrator/releases) からお使いの OS に合ったファイルをダウンロードしてください。

> **パソコンが苦手な方へ**: Releases ページを開いたら、下にスクロールして「Assets」をクリックし、下の表を参考にファイルを 1 つだけダウンロードしてください。

#### 📥 ダウンロードするファイル（OS 別）

| OS | ファイル名 | 説明 | おすすめ |
|----|-----------|------|:--------:|
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_en-US.msi` | Windows インストーラー (.msi / English)。ダブルクリックで実行 | |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_ja-JP.msi` | Windows インストーラー (.msi / 日本語) | ⭐ |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_zh-CN.msi` | Windows インストーラー (.msi / 中文) | |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64-setup.exe` | Windows インストーラー (.exe / NSIS)。.msi が動かない場合に使用 | |
| **macOS** | `Zero-Employee.Orchestrator_0.1.0_universal.dmg` | macOS (Intel / Apple Silicon) 用。開いて Applications にドラッグ | ⭐ |
...
| **Linux** | `Zero-Employee.Orchestrator-0.1.0-1.x86_64.rpm` | Fedora / RHEL 用パッケージ | |

#### 🚫 ダウンロード不要なファイル（自動で使われるもの）

以下のファイルは**アプリの自動更新機能が内部的に使うもの**です。手動ダウンロードは不要です。

| ファイル名 | 用途 |
|-----------|------|
| `*.sig` ファイル（8 個） | 自動更新時の署名検証用。改ざん防止のために使用 |
| `latest.json` | アプリが新しいバージョンの有無を確認するメタデータ |
| `*_universal.app.tar.gz` | macOS 自動更新用アーカイブ |

> **まとめ**: Assets に 18 個のファイルがありますが、**あなたがダウンロードするのは OS に合った 1 つだけ**です。

> **最新版**: [こちら](https://github.com/OrosiTororo/Zero-Employee-Orchestrator/releases/latest) からダウンロードできます。

#### CLI / TUI 版（エンジニア向け）

> **注意**: このパッケージは PyPI に公開されていません。ソースからインストールしてください。

```bash
git clone https://github.com/OrosiTororo/Zero-Employee-Orchestrator.git
cd Zero-Employee-Orchestrator

# 方法 1: リポジトリルートからインストール
pip install .

# 方法 2: API パッケージを直接インストール
...
| **Brainstorming (Sparring Partner)** | Brainstorm with an AI advisor. Compare responses from multiple models side-by-side |
| **Multi-Model Comparison** | Send the same input to GPT / Gemini / Claude etc. and compare responses |
| **Conversation Memory** | Permanently store all conversations between users and the AI organization. Reference past discussions |
| **Per-Role Model Settings** | Users can freely assign AI models to each agent role |
| **Accurate Character Counting** | Unicode-aware character counting supporting Japanese, Chinese, and more |
| **Dynamic Agent Management** | Users can add, remove, and modify agent roles like Secretary and Advisor |
| **Natural Language Org Management** | Manage the AI organization with natural language like "Add an advisor agent" |
| **File Attachment-based Planning** | Attach files to Design Interview and integrate into spec generation context |
...
Download the file for your OS from the [Releases page](https://github.com/OrosiTororo/Zero-Employee-Orchestrator/releases).

> **Non-technical users**: Open the Releases page, scroll down, click "Assets", and download **just one file** that matches your OS from the table below.

#### 📥 Files to Download (by OS)

| OS | File | Description | Recommended |
|----|------|-------------|:-----------:|
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_en-US.msi` | Windows installer (.msi / English). Double-click to run | ⭐ |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_ja-JP.msi` | Windows installer (.msi / Japanese) | |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_zh-CN.msi` | Windows installer (.msi / Chinese) | |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64-setup.exe` | Windows installer (.exe / NSIS). Use if .msi doesn't work | |
| **macOS** | `Zero-Employee.Orchestrator_0.1.0_universal.dmg` | macOS (Intel / Apple Silicon). Open and drag to Applications | ⭐ |
| **Linux** | `Zero-Employee.Orchestrator_0.1.0_amd64.AppImage` | Runs directly (no installation needed) | ⭐ |
| **Linux** | `Zero-Employee.Orchestrator_0.1.0_amd64.deb` | Debian / Ubuntu package | |
| **Linux** | `Zero-Employee.Orchestrator-0.1.0-1.x86_64.rpm` | Fedora / RHEL package | |

#### 🚫 Files You Do NOT Need to Download (used automatically)

These files are **used internally by the app's auto-update system**. No manual download required.

| File | Purpose |
|------|---------|
| `*.sig` files (8) | Signature verification for auto-updates. Prevents tampering |
| `latest.json` | Metadata for the app to check for new versions |
| `*_universal.app.tar.gz` | macOS auto-update archive |

> **Summary**: There are 18 files in Assets, but **you only need to download one** that matches your OS.

> **Latest**: Download from [here](https://github.com/OrosiTororo/Zero-Employee-Orchestrator/releases/latest).

#### CLI / TUI (For Engineers)

> **Note**: This package is not published on PyPI. Install from source.

```bash
git clone https://github.com/OrosiTororo/Zero-Employee-Orchestrator.git
cd Zero-Employee-Orchestrator

# Option 1: Install from repo root
pip install .

# Option 2: Install the API package directly
pip install apps/api/

# Option 3: Using uv (dev mode)
uv pip install -e apps/api/
```

After installation, CLI commands become available:

```bash
zero-employee serve          # Start the API server
zero-employee local          # Local chat mode (Ollama)
zero-employee config list    # List configuration values
...
│   │   │   ├── api/
│   │   │   │   ├── routes/     # REST API Endpoints (22 routes)
│   │   │   │   ├── ws/         # WebSocket
...
| **头脑风暴（对话伙伴）** | 与AI顾问进行头脑风暴。可同时比较多个模型的回答 |
| **多模型对比** | 向 GPT / Gemini / Claude 等发送相同输入，并排比较回答 |
| **对话记忆** | 永久保存用户与AI组织的所有对话记录。可引用过去的讨论 |
| **按角色设置模型** | 用户可自由为每个代理角色分配AI模型 |
| **精确字符计数** | 支持日语、中文等Unicode字符的精确计数 |
| **动态代理管理** | 用户可添加、删除、修改秘书和顾问等代理角色 |
| **自然语言组织管理** | 用自然语言管理AI组织，如"添加一个顾问代理" |
| **基于文件附件的计划创建** | 在 Design Interview 中附加文件，集成到规格书生成的上下文中 |
| **审批流程** | 危险操作必须获得人类审批 |
| **审计日志** | 所有重要操作可追溯 |
...
从 [Releases 页面](https://github.com/OrosiTororo/Zero-Employee-Orchestrator/releases) 下载与您操作系统匹配的文件。

> **非技术用户**：打开 Releases 页面，向下滚动，点击 "Assets"，根据下表下载**一个文件**即可。

#### 📥 需要下载的文件（按操作系统）

| 操作系统 | 文件名 | 说明 | 推荐 |
|---------|--------|------|:----:|
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_en-US.msi` | Windows 安装程序 (.msi / English)。双击运行 | |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_ja-JP.msi` | Windows 安装程序 (.msi / 日本語) | |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_zh-CN.msi` | Windows 安装程序 (.msi / 中文) | ⭐ |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64-setup.exe` | Windows 安装程序 (.exe / NSIS)。.msi 无法运行时使用 | |
| **macOS** | `Zero-Employee.Orchestrator_0.1.0_universal.dmg` | macOS (Intel / Apple Silicon) 用。打开后拖到 Applications | ⭐ |
...
| **Linux** | `Zero-Employee.Orchestrator-0.1.0-1.x86_64.rpm` | Fedora / RHEL 软件包 | |

#### 🚫 无需手动下载的文件（自动使用）

以下文件由**应用的自动更新系统内部使用**，无需手动下载。

| 文件名 | 用途 |
|--------|------|
| `*.sig` 文件（8 个） | 自动更新时的签名验证，防止篡改 |
| `latest.json` | 应用检查新版本时使用的元数据 |
| `*_universal.app.tar.gz` | macOS 自动更新归档 |

> **总结**：Assets 中有 18 个文件，但**您只需要下载与您操作系统匹配的 1 个**。

> **最新版本**：从[这里](https://github.com/OrosiTororo/Zero-Employee-Orchestrator/releases/latest)下载。

#### CLI / TUI版（面向工程师）

> **注意**：此包未发布到 PyPI。请从源码安装。
...
zero-employee config list    # 显示设置值列表
zero-employee models         # 可用模型列表
