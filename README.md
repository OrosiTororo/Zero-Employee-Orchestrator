...
> **v0.1.1 — AI Orchestration Platform — Design · Execute · Verify · Improve**
>
...
| OS | ファイル名 | 説明 | おすすめ |
|----|-----------|------|:--------:|
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_en-US.msi` | Windows インストーラー (.msi)。ダブルクリックで実行 | ⭐ |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64-setup.exe` | Windows インストーラー (.exe / NSIS)。.msi が動かない場合に使用 | |
| **macOS** | `Zero-Employee.Orchestrator_0.1.0_aarch64.dmg` | macOS (Apple Silicon M1/M2/M3) 用。開いて Applications にドラッグ | ⭐ |
...
| ファイル名 | 用途 |
|-----------|------|
| `*.sig` ファイル（6 個） | 自動更新時の署名検証用。改ざん防止のために使用 |
| `latest.json` | アプリが新しいバージョンの有無を確認するメタデータ |
| `*_aarch64.app.tar.gz` | macOS 自動更新用アーカイブ |

> **まとめ**: Assets に 14 個のファイルがありますが、**あなたがダウンロードするのは OS に合った 1 つだけ**です。
...
|----|------|-------------|:-----------:|
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_en-US.msi` | Windows installer (.msi). Double-click to run | ⭐ |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64-setup.exe` | Windows installer (.exe / NSIS). Use if .msi doesn't work | |
| **macOS** | `Zero-Employee.Orchestrator_0.1.0_aarch64.dmg` | macOS (Apple Silicon M1/M2/M3). Open and drag to Applications | ⭐ |
...
| File | Purpose |
|------|---------|
| `*.sig` files (6) | Signature verification for auto-updates. Prevents tampering |
| `latest.json` | Metadata for the app to check for new versions |
| `*_aarch64.app.tar.gz` | macOS auto-update archive |

> **Summary**: There are 14 files in Assets, but **you only need to download one** that matches your OS.
...
| 操作系统 | 文件名 | 说明 | 推荐 |
|---------|--------|------|:----:|
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64_en-US.msi` | Windows 安装程序 (.msi)。双击运行 | ⭐ |
| **Windows** | `Zero-Employee.Orchestrator_0.1.0_x64-setup.exe` | Windows 安装程序 (.exe / NSIS)。.msi 无法运行时使用 | |
| **macOS** | `Zero-Employee.Orchestrator_0.1.0_aarch64.dmg` | macOS (Apple Silicon M1/M2/M3) 用。打开后拖到 Applications | ⭐ |
...
| 文件名 | 用途 |
|--------|------|
| `*.sig` 文件（6 个） | 自动更新时的签名验证，防止篡改 |
| `latest.json` | 应用检查新版本时使用的元数据 |
| `*_aarch64.app.tar.gz` | macOS 自动更新归档 |

> **总结**：Assets 中有 14 个文件，但**您只需要下载与您操作系统匹配的 1 个**。
