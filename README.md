...
>
> ChatGPT Plus・Gemini Advanced・Claude Pro などのサブスクリプションは、Web サイトやスマートフォンアプリ向けのサービスです。**プログラムから直接呼び出すことはできません。**
> プログラム用の API アクセスは別途取得・課金が必要です。

### 無料で始める方法

| 方法 | 説明 |
|------|------|
| **Google Gemini（無料枠）** | Google AI Studio でキーを取得。無料枠あり・クレジットカード不要 |
| **Ollama（ローカル LLM）** | PC 上で LLM を実行。API キー不要・完全オフライン・無制限 |
...
- `ABOUT.md` — **このシステムのメリット・従来システムとの違い（日本語）**
- `USER_GUIDE.md` — **初心者向けユーザーガイド（日本語）**
