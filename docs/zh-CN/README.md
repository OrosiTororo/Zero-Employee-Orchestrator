...
[English](../../README.md) | [日本語](../ja-JP/README.md) | [**简体中文**](README.md) | [繁體中文](../zh-TW/README.md) | [한국어](../ko-KR/README.md) | [Português (Brasil)](../pt-BR/README.md) | [Türkçe](../tr/README.md)
...
<tr>
<td align="center"><b>快速入门指南</b><br/>安装、首次工作流、CLI 基础。<b>请先阅读此文档。</b></td>
<td align="center"><b>架构深度解析</b><br/>9 层架构、DAG 编排、Judge Layer、Experience Memory。</td>
<td align="center"><b>安全指南</b><br/>提示注入防御、审批门控、IAM、沙箱、PII 保护。</td>
</tr>
</table>

| 主题 | 学习内容 |
|------|---------|
| 9 层架构 | User → Design Interview → Task Orchestrator → Skill → Judge → Re-Propose → Memory → Provider → Registry |
| Self-Healing DAG | 任务失败时自动重新规划和重新提案 |
| Judge Layer | 基于规则的初判 + 跨模型高精度验证 |
| Skill / Plugin / Extension | 支持自然语言生成技能的三层可扩展体系 |
| Human-in-the-Loop | 12 类危险操作需要人类审批 |
| 安全优先设计 | 提示注入防御（40+ 模式）、PII 脱敏、文件沙箱 |
...
- **9 层架构** — User Layer → Design Interview → Task Orchestrator → Skill Layer → Judge Layer → Re-Propose → State & Memory → Provider → Skill Registry
- **Self-Healing DAG** — 任务失败时通过动态 DAG 重构自动重新规划
- **Judge Layer** — 基于规则的初判 + 跨模型高精度验证
- **Experience Memory** — 从历史执行中学习，提升未来性能
- **Skill / Plugin / Extension** — 三层可扩展体系：8 个内置技能、10 个插件、5 个扩展
- **自然语言技能生成** — 用自然语言描述技能，AI 自动生成（含安全性检查）
- **浏览器辅助** — Chrome 扩展程序叠加聊天，实时屏幕共享和错误诊断
- **媒体生成** — 图像（DALL-E, SD）、视频（Runway ML, Pika）、音频（TTS, ElevenLabs）、音乐（Suno）、3D（动态提供商注册）
- **AI 工具集成** — 25+ 外部工具（GitHub、Slack、Jira、Figma 等）可由 AI 操作
- **安全优先** — 提示注入防御（5 个类别、40+ 模式）、审批门控、IAM、PII 保护、文件沙箱
- **多模型支持** — 通过 `model_catalog.json` 实现动态模型目录，已弃用模型自动回退
- **多语言（i18n）** — 日本語 / English / 中文 — 界面、AI 回复和 CLI 无缝切换
- **自主运行** — Docker / Cloudflare Workers 实现 24/365 后台执行
...
| 操作系统 | 文件 | 说明 |
|---|---|---|
| **Windows** | `.msi` / `-setup.exe` | Windows 安装程序 |
| **macOS** | `.dmg` | macOS (Intel / Apple Silicon) |
| **Linux** | `.AppImage` | 便携式（无需安装） |
| **Linux** | `.deb` / `.rpm` | Debian/Ubuntu / Fedora/RHEL |

所有安装程序均包含**设置向导**，可选择语言（English / 日本語 / 中文 / 한국어 / Português / Türkçe）。语言可随时在**设置**中更改。
...
✨ **完成了！** 您现在拥有了一个具备人类审批门控和审计功能的完整 AI 编排平台。
...
语言设置在系统范围内生效：CLI 输出、AI 回复和 Web 界面会同时切换。
...
| **AI 工具集成** | 25+ 外部工具（GitHub、Slack、Jira、Figma 等） |
| **A2A 通信** | 代理间点对点消息、频道和协商 |
| **分身 AI** | 从用户对话中学习判断标准，共同成长 |
| **秘书 AI** | 脑暴 → 结构化任务，作为用户和 AI 组织的桥梁 |
