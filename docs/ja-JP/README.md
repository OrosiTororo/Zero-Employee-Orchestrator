...
ZEO は既存の AI ツールを置き換えるのではなく、統合します。CrewAI、AutoGen、LangChain、Dify、Claude Cowork、n8n、Zapier、そして 34 以上のビジネスアプリを、単一の承認ゲート、監査証跡、セキュリティレイヤーの下に接続します。自然言語で業務ワークフローを定義し、複数の AI エージェントを役割分担させ、人間の承認ゲートと完全な監査可能性を備えた状態でタスクを実行します。Self-Healing DAG・Judge Layer・Experience Memory を備えた 9 層アーキテクチャで構築されています。
...
| 方法 | 対象 | 所要時間 | API キー |
|------|------|----------|----------|
| **[デスクトップアプリ](#️-デスクトップアプリのダウンロード)** | 非技術者 | 2 分 | 不要（サブスクリプションモード） |
| **[CLI (pip install)](#-クイックスタート-cli)** | 開発者 | 2 分 | 不要（サブスクリプション or Ollama） |
| **[Docker](#-docker)** | セルフホスト / 本番 | 5 分 | 不要（サブスクリプション or Ollama） |

**システム要件:** Python 3.11+（CLI）、Node.js 22+（フロントエンド開発）、RAM 4 GB 以上。Ollama ローカルモデルは 8 GB 以上の RAM が必要。
...
| OS | ファイル | 説明 |
|---|---|---|
| **Windows** | `-setup.exe` | Windows インストーラー (x64) |
| **macOS** | `.dmg` | macOS Universal (Intel + Apple Silicon) |
| **Linux** | `.AppImage` | ポータブル（インストール不要、amd64） |
| **Linux** | `.deb` / `.rpm` | Debian/Ubuntu / Fedora/RHEL (amd64/x86_64) |
| **Chrome** | `chrome-extension.zip` | Browser Assist (chrome://extensions) |

インストール後、**セットアップウィザード**が以下の手順を案内します：
1. **言語** — English、日本語、中文、한국어、Português、Türkçe から選択（設定で後から変更可能）
2. **LLM プロバイダー** — AI の実行方法を選択（サブスクリプションモードなら API キー不要）
3. **最初のタスク** — すぐにプラットフォームを使い始める
...
```bash
# 方法 A: API キー不要 — g4f 経由の無料 Web AI サービスを利用
zero-employee config set DEFAULT_EXECUTION_MODE subscription

# 方法 B: 完全オフライン — Ollama によるローカルモデル（インターネット不要）
zero-employee config set DEFAULT_EXECUTION_MODE free
zero-employee pull qwen3:8b

# 方法 C: API キー — 最高品質、プロバイダーへの従量課金
zero-employee config set OPENROUTER_API_KEY <your-key>  # or GEMINI_API_KEY, etc.
```

> **ZEO 自体は無料です。** LLM の API 費用（発生する場合）はユーザーが各プロバイダーに直接支払います。詳しくは [USER_SETUP.md](../../USER_SETUP.md) を参照してください。

### ステップ 3: 起動

```bash
# 方法 A: start.sh（バックエンド + フロントエンドを自動で起動）
./start.sh
# → http://localhost:5173 を開く

# 方法 B: 手動で起動
zero-employee serve              # API サーバーを起動（ポート 18234）
cd apps/desktop/ui && pnpm dev   # 別のターミナルでフロントエンドを起動（ポート 5173）
# → http://localhost:5173 を開く

# 方法 C: チャットモードのみ（Web UI 不要）
zero-employee chat               # デフォルト設定
zero-employee local --model qwen3:8b  # Ollama
```

> **注意:** `zero-employee serve` は API サーバーのみを起動します。Web UI は別途ポート 5173 で動作します。最も簡単な方法は `start.sh` を使用することです。

### ステップ 4: 動作確認

```bash
zero-employee health              # サーバー状態を確認
zero-employee models              # 利用可能なモデルを一覧表示
zero-employee config list         # 設定を確認
```

### 言語の切り替え

デフォルト言語は英語です。以下の方法でシステム全体（CLI・AI 応答・Web UI）を一括で切り替えられます：
...
デスクトップアプリでは、**設定**からいつでも言語を変更できます。
...
<tr>
<td align="center"><b>クイックスタートガイド</b><br/>初回ワークフロー、CLI の基本。</td>
<td align="center"><b>アーキテクチャ詳解</b><br/>9 層アーキテクチャ、DAG、Judge Layer。</td>
<td align="center"><b>セキュリティガイド</b><br/>プロンプト防御、承認ゲート、サンドボックス。</td>
</tr>
</table>
...
│   │       ├── api/routes/         # 53 REST API ルートモジュール（432 エンドポイント）
│   │       ├── api/ws/             # WebSocket
...
| **App Connector Hub** | 65+ アプリ（Obsidian、Notion、Google Workspace、Microsoft 365、Slack 等） — コネクタのコネクタ |
...
| **アプリコネクタハブ** | 65+ アプリ（Obsidian、Notion、Google Workspace、Microsoft 365 等） |
| **AI ツール統合** | 21 カテゴリ、55+ の外部ツール |
| **A2A 通信** | エージェント間のピアツーピア通信・チャンネル・交渉 |
| **分身 AI** | ユーザーの判断基準を学習し共に成長 |
| **秘書 AI** | ブレインダンプ → 構造化タスク、AI 組織との橋渡し |
| **オペレータープロファイル** | Cowork スタイルの自己紹介 + グローバル指示 — AI があなたの役割・優先事項・作業スタイルに基づいて応答をパーソナライズ |
| **タスクディスパッチ** | Cowork Dispatch にインスパイアされたバックグラウンドタスク — 発行して忘れる方式でステータスポーリング対応 |
...
| **レッドチームセキュリティ** | 8 カテゴリ・20+ テストで自己脆弱性を定期検査 |
...
| **承認ゲート** | 14 カテゴリの危険操作（送信・削除・課金・権限変更等）に人間の承認を必須化 |
| **自律実行境界** | AI が自律実行できる操作を明示的に制限 |
| **IAM & ツール権限** | 人間/AI アカウント分離、ロールベースのツール権限（5 つのデフォルトポリシー: secretary, researcher, reviewer, executor, admin）でエージェントごとに最小権限を適用 |
| **キルスイッチ** | UI ボタンまたは API（`/kill-switch/activate`）経由で全アクティブ実行を緊急停止。再開するまで新規実行をブロック |
| **段階的 Judge** | 3 段階検証: LIGHTWEIGHT（ルールのみ）→ STANDARD（+ポリシー）→ HEAVY（+クロスモデル）。低リスク操作のコストを削減しつつ、高リスク操作では完全な検証を維持 |
| **メモリ信頼度** | Experience Memory エントリはソースタイプ、信頼レベル（0.0-1.0）、検証ステータス、有効期限を追跡。信頼できるメモリ（0.7 以上、未期限切れ）のみが使用される |
...
18 種類の危険パターンを自動検出。安全性チェック通過後にのみ登録。
...
- **リアルタイム画面共有**: 手動でスクリーンショットを撮らずに AI が画面を確認
