...
- 初回起動時のセットアップウィザードで言語を選択（日本語 / English / 中文）
- 設定画面からいつでも言語を切替可能
...
| 検査 | 説明 |
|------|------|
| **プロンプトインジェクション** | 40+ パターンによる 5 段階脅威検出 |
...
日本語・英語・中国語の 3 言語に対応。UI 表示、AI への指示、CLI 出力すべてが選択言語に切り替わります。
...
| フロントエンド UI | Zustand + JSON ロケール（320+ キー） | `shared/i18n/locales/{ja,en,zh}.json` |
