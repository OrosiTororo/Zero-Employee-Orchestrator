...
> [English](../FEATURES.md) | 日本語 | [中文](../zh/FEATURES.md)
...
## v0.1.1 の新機能 (2026-03-28)

| 分野 | 変更内容 |
|------|---------|
| **本番デプロイ** | Docker Compose にリソース制限、ネットワーク分離、読み取り専用 FS、ログローテーションを追加 |
| **CI セキュリティ** | Trivy コンテナスキャン、ブロッキング pip-audit、Red-team テストジョブを追加 |
| **Red-team テスト** | 全 22 テストが実際のセキュリティモジュールを実ペイロードで実行 |
| **多言語対応** | 6 言語で 65+ 翻訳キー（EN/JA/ZH/KO/PT/TR）、デフォルト言語を英語に変更 |
| **モデルカタログ** | GPT、Llama、Phi、Claude Haiku の最新版に更新 |
| **PII ガード** | 全 13 カテゴリに検出パターン完備（運転免許証、日本語氏名を追加） |
| **ドキュメント** | 全ての誇大数値を修正、7 言語版で一貫性を確保 |
...
28. [外部ツール連携](#28-外部ツール連携-v01)
29. [コミュニティプラグイン共有](#29-コミュニティプラグイン共有-v01)
30. [AI Self-Improvement](#30-ai-self-improvement--level-2-自己改善の芽-v01)
31. [v0.1 機能肥大化レビュー](#31-v01-機能肥大化レビュー--コアと拡張の境界)
32. [メタスキル概念](#32-メタスキル概念-v01)
33. [ファイル添付による計画作成](#33-ファイル添付による計画作成-v01)
34. [セキュリティ強化](#34-セキュリティ強化-v01)
35. [RSS/ToS 自動更新パイプライン](#35-rstos-自動更新パイプライン-v01)
36. [Knowledge Refresh](#36-knowledge-refresh--コンテキストウィンドウ管理-v01)
37. [A2A 双方向通信](#37-a2a-双方向通信-v01)
38. [Avatar AI Co-evolution](#38-avatar-ai-co-evolution--ユーザーとの共進化-v01)
39. [Longrun Scheduler](#39-longrun-scheduler--24365-長期継続実行-v01)
40. [Agent Session](#40-agent-session--コンテキスト永続化-v01)
41. [Artifact Bridge](#41-artifact-bridge--成果物の工程間連携-v01)
42. [メディア生成統合](#42-メディア生成統合-v01)
43. [AI ツールレジストリ](#43-ai-ツールレジストリ-v01)
44. [iPaaS 統合](#44-ipaas-統合--ワークフロー連携-v01)
45. [アーティファクトエクスポート](#45-アーティファクトエクスポート-v01)
46. [AI 共創リパーパスエンジン](#46-ai-共創リパーパスエンジン-v01)
47. [Obsidian 統合](#47-obsidian-統合-v01)
48. [クラウドサービスネイティブ統合](#48-クラウドサービスネイティブ統合-v01)
49. [スマートデバイス / VR/AR 統合](#49-スマートデバイス--vrar-統合-v01)
50. [ガバナンス・コンプライアンス](#50-ガバナンスコンプライアンス-v01)
51. [Skill マーケットプレイス](#51-skill-マーケットプレイス-v01)
52. [マルチユーザー・チーム管理](#52-マルチユーザーチーム管理-v01)
53. [Red-team セキュリティテスト](#53-red-team-セキュリティテスト-v011)
54. [ワークスペース隔離](#54-ワークスペース隔離-v01)
55. [Design Interview 過去失敗パターンフィードバック](#55-design-interview-過去失敗パターンフィードバック-v01)
56. [前提変化の汎用監視 — Prerequisite Monitor](#56-前提変化の汎用監視--prerequisite-monitor-v01)
57. [Spec 間矛盾検出](#57-spec-間矛盾検出--spec-contradiction-detector-v01)
58. [タスク実行のリプレイ・比較](#58-タスク実行のリプレイ比較--task-replay--comparison-v01)
59. [ユーザー判断の振り返りレポート](#59-ユーザー判断の振り返りレポート--judgment-review-v01)
60. [目的→Plan 分解の品質検証](#60-目的plan-分解の品質検証--plan-quality-verifier-v01)
61. [マルチモデル比較](#61-マルチモデル比較-v01)
62. [ブレインストーミングセッション](#62-ブレインストーミングセッション-v01)
63. [会話メモリ — Conversation Memory](#63-会話メモリ--conversation-memory-v01)
64. [テキスト解析サービス](#64-テキスト解析サービス-v01)
65. [ロールベースモデル設定](#65-ロールベースモデル設定-v01)
66. [AI 組織動的管理](#66-ai-組織動的管理-v01)
67. [カスタムエージェントロール](#67-カスタムエージェントロール-v01)
68. [自然言語フィーチャーリクエスト](#68-自然言語フィーチャーリクエスト-v01)
69. [ブレインダンプ / Secretary AI](#69-ブレインダンプ--secretary-ai-v01)
70. [ファイルアップロード管理](#70-ファイルアップロード管理-v01)
71. [タスク実行中のユーザー入力](#71-タスク実行中のユーザー入力-v01)
72. [リソースインポート](#72-リソースインポート-v01)
73. [プロジェクト・ゴール管理](#73-プロジェクトゴール管理-v01)
74. [仮説エンジン — Hypothesis Engine](#74-仮説エンジン--hypothesis-engine-v01)
75. [AI 調査エンジン — AI Investigator](#75-ai-調査エンジン--ai-investigator-v01)
76. [ブラウザ自動操作 — Browser Automation](#76-ブラウザ自動操作--browser-automation-v01)
77. [LSP 統合 — Language Server Protocol](#77-lsp-統合--language-server-protocol-v01)
78. [MCP 統合 — Model Context Protocol](#78-mcp-統合--model-context-protocol-v01)
79. [外部スキル検索・インポート](#79-外部スキル検索インポート-v01)
80. [入力セキュリティミドルウェア](#80-入力セキュリティミドルウェア-v01)
81. [多言語対応 (i18n)](#81-多言語対応-i18n-v01)
...
|--------|-----------|-------------|-----------|------------|--------------|
| **DRAFT** | GPT Mini, Claude Haiku | 1 回 | 50% | 不要 | なし |
| **STANDARD** | GPT, Claude Sonnet | 2 回 | 70% | 不要 | なし |
| **HIGH** | GPT, Claude Sonnet | 3 回 | 85% | 不要 | **あり** |
| **CRITICAL** | Claude Opus, GPT | 5 回 | 95% | **必須** | **あり** |
...
| プロバイダー | 対応モデル例 |
|-------------|-------------|
| **OpenRouter** | 複数モデルを単一 API キーで利用 |
| **OpenAI** | GPT, GPT Mini |
| **Anthropic** | Claude Opus, Sonnet, Haiku |
| **Google** | Gemini Pro, Flash, Flash Lite |
| **DeepSeek** | DeepSeek Chat |
| **Ollama** | Llama, Mistral, Phi, Qwen 等（ローカル無料） |
...
| モード | 説明 | 推奨モデル |
|--------|------|-----------|
| `QUALITY` | 最高品質 | Claude Opus, GPT |
| `SPEED` | 高速応答 | Claude Haiku, GPT Mini |
| `COST` | 低コスト | Claude Haiku, GPT Mini, DeepSeek |
...
- **カスタムモデル登録** — デフォルトカタログにないLLMモデルを `POST /api/v1/models/add` で追加可能（LiteLLM, OpenRouter, Ollama, vLLM, カスタムエンドポイント対応）
- **レガシーモデルサポート** — 廃止済みだが稼働中のモデル（GPT-4o, Gemini 2.5 等）を `?tag=legacy&include_deprecated=true` で利用可能
...
- Ollama モデル自動検出
- モデル削除: `DELETE /api/v1/models/{model_id}`
...
| **設定** | 言語切替・API キー・実行モード・企業設定・プロバイダー接続 |
| **リリース** | バージョン履歴 |
...
`/api/v1` プレフィックスの下に 41 ルートモジュール・350 以上のエンドポイントを提供します。
...
| OS | 形式 |
|----|------|
| Windows | `.exe`（セットアップインストーラー） |
| macOS | `.dmg`（Intel + Apple Silicon ユニバーサル） |
| Linux | `.AppImage` / `.deb` / `.rpm` |

- Python バックエンドはサイドカーとして同梱
- ローカルファイルアクセス、セッション管理、UI をローカルで実行
- LLM API や外部 SaaS はクラウド経由
- 初回起動時のセットアップウィザードで言語を選択（日本語 / English / 中文 / 한국어 / Português / Türkçe）
- 設定画面からいつでも言語を切替可能
...
| `mcp` | Model Context Protocol | Claude Desktop、IDE 連携 |
...
## 35. RSS/ToS 自動更新パイプライン (v0.1)

AI プロバイダーの RSS フィード・利用規約・料金ページを定期的に監視し、変更を自動検出します。

### 監視対象プロバイダー

| プロバイダー | RSS | ToS | 料金 | チェック間隔 |
|-------------|-----|-----|------|-------------|
| **OpenAI** | ✅ | ✅ | ✅ | 12 時間 |
| **Anthropic** | ✅ | ✅ | ✅ | 12 時間 |
| **Google AI** | ✅ | ✅ | ✅ | 12 時間 |
...
| 変更タイプ | 影響レベル | 説明 |
|-----------|-----------|------|
| `MODEL_UPDATE` | MEDIUM | 新モデルリリース・バージョン更新 |
| `PRICING_CHANGE` | MEDIUM | 料金変更 |
| `TOS_UPDATE` | HIGH | 利用規約改定 |
| `NEW_FEATURE` | LOW | 新機能追加 |
| `DEPRECATION` | HIGH | モデル・API の廃止予定 |
| `API_CHANGE` | HIGH | API の破壊的変更 |
| `SECURITY_ADVISORY` | CRITICAL | セキュリティアドバイザリ |

### 自動更新フロー

1. RSS フィード・ToS ページ・料金ページを定期取得
2. ハッシュ比較で変更を検出
3. キーワード分類で変更タイプを判定
4. 影響レベルを自動評価（緊急キーワードで CRITICAL にエスカレーション）
5. `MODEL_UPDATE` 検出時は `ModelRegistry.refresh_catalog()` を自動トリガー
6. カスタム監視サービスの追加も可能

---

## 36. Knowledge Refresh — コンテキストウィンドウ管理 (v0.1)

LLM のコンテキストウィンドウ制限に対応し、タスク実行に必要な知識のみを動的に選択・注入します。

### Knowledge Pipeline

```
取得 → 抽出 → 分割 → インデックス化 → 検索 → 引用・要約 → 検証済み知識への昇格/却下
```
...
| 種類 | 説明 |
|------|------|
| `conversation_log` | 会話履歴 |
| `reusable_improvement` | 再利用可能な改善知識 |
| `experimental_knowledge` | 実験的知識 |
| `verified_knowledge` | 検証済み知識 |
| `experience_memory` | 成功パターン |
| `failure_taxonomy` | 失敗分類 |
| `policy_memory` | 承認条件・禁止事項 |
| `skill_improvement` | Skill 改善知識 |
| `plugin_operation` | Plugin 運用ノウハウ |

### 知識ライフサイクル

```
RAW → EXTRACTED → INDEXED → VERIFIED (承認済み)
                           → EXPERIMENTAL (実験的)
                           → REJECTED (却下)
```

- トークン推定に基づく知識の選択と切り詰め
- 検証済み知識のみをタスク実行コンテキストに注入
- 知識の昇格・却下はユーザー承認を経由
...
エージェント間のピアツーピア通信を実現するハブです。親→子のサブエージェント指示だけでなく、対等な通信・交渉・チャネルベースのブロードキャストをサポートします。

### 機能

| 機能 | 説明 |
|------|------|
| **ダイレクトメッセージ** | エージェント間の 1:1 メッセージ送受信 |
| **チャネル通信** | 名前付きチャネルによるグループ通信・ブロードキャスト |
| **スレッド追跡** | メッセージスレッドの自動グループ化 |
| **交渉プロトコル** | エージェント間の合意形成プロトコル |
| **優先度制御** | LOW / NORMAL / HIGH / URGENT のメッセージ優先度 |

---

## 38. Avatar AI Co-evolution — ユーザーとの共進化 (v0.1)

ユーザーのインタラクションパターンから意思決定基準を学習し、AI がユーザーと共に成長する共進化エンジンです。

### 機能

| 機能 | 説明 |
|------|------|
| **インタラクション記録** | ユーザーの承認・却下・編集操作を記録 |
| **嗜好抽出** | 操作パターンからユーザー嗜好を自動抽出 |
| **意思決定基準の学習** | 承認/却下の判断基準を蓄積 |
| **選択肢ランキング** | ユーザー嗜好に基づく提案の優先度付け |
| **ユーザーモデルの管理** | エクスポート/インポート対応 |

---

## 39. Longrun Scheduler — 24/365 長期継続実行 (v0.1)

AI 組織が 24 時間 365 日継続的に稼働するためのスケジューラです。
...
## 40. Agent Session — コンテキスト永続化 (v0.1)

AI エージェントが複数ラウンドのやり取りでコンテキストを保持したまま作業を継続できるセッション管理機能です。

### 機能

- セッション状態管理: ACTIVE / IDLE / EXPIRED / TERMINATED
- コンテキストの DB 永続化（SQLAlchemy async）
- idle 状態での待機とコンテキスト保持
- セッションの自動期限切れ

---

## 41. Artifact Bridge — 成果物の工程間連携 (v0.1)

工程間で成果物を受け渡し、バージョン管理と再利用を可能にします。
...
| **互換性マトリクス** | アーティファクトタイプ間の互換性マッピング |
| **アーティファクト検索** | 互換アーティファクトの検索 |
| **パイプラインサポート** | スキルチェーンのアーティファクトフロー構築 |

---

## 42. メディア生成統合 (v0.1)

外部 API を利用した画像・動画・音声・音楽・3D モデルの生成を統合します。
...
| **音楽生成** | Suno, Udio |
| **3D ほか** | ユーザーが API から動的登録可能 |

### 動的プロバイダーレジストリ

ビルトインの 10 プロバイダーに加え、ユーザーが API 経由で新しいプロバイダーを登録・削除可能です。3D モデリングツール（Meshy, Spline 等）や新しい画像・動画生成サービスをコード変更なしに追加できます。

- `POST /api/v1/media/providers` — 新規プロバイダー登録
- `DELETE /api/v1/media/providers/{id}` — ユーザー登録プロバイダーの削除
- ビルトインプロバイダーは削除・上書き不可（安全性保護）
- `MediaType` に `3d` を追加対応

安全性: プロンプトインジェクション検査、承認ゲート経由、データ保護ポリシー適用、監査ログ記録。

API: `/api/v1/media/*`

---

## 42a. タスク単位のプロバイダー指定 (v0.1)

タスクごとに使用する LLM プロバイダー・モデル・実行モードを指定可能です。

### 仕組み

- **`provider_override`**: タスク作成時または既存タスクへの PATCH で指定
- **フォールバック**: タスク指定 → 会社デフォルト の優先順位で解決
- **実行監視連携**: タスク開始時に `ExecutionMonitor` へ解決済みプロバイダー情報を通知
- **監査ログ**: `provider_override` が audit_logs に記録される

### API

- `POST /api/v1/plans/{plan_id}/tasks` — `provider_override` フィールドで指定
- `PATCH /api/v1/tasks/{task_id}/provider` — 既存タスクのプロバイダーを変更（pending/ready/blocked のみ）

### provider_override の構造
...
すべてのフィールドはオプション。未指定のフィールドは会社デフォルトにフォールバック。

---

## 43. AI ツールレジストリ (v0.1)

AI エージェントが操作可能な 25 以上の外部ツールを統合管理します。

### 対応ツールカテゴリ

| カテゴリ | ツール例 |
|---------|---------|
| コード関連 | GitHub, GitLab, Bitbucket, コードレビュー |
| ドキュメント | Google Docs, Notion, Confluence, Obsidian |
| コミュニケーション | Slack, Discord, LINE, Email |
| プロジェクト管理 | Jira, Linear, Asana, Trello |
| デザイン | Figma (MCP 経由) |
| データ分析 | Google Sheets, Airtable |
| クラウド | AWS, GCP, Azure (CLI 経由) |
| 検索 | Web Search, RAG, Knowledge Base |
| メディア生成 | 画像・動画・音声・音楽・3D (media_generation.py、動的プロバイダー登録対応) |
| ブラウザ操作 | Browser Assist, Playwright |

すべてのツール操作は承認ゲート経由・監査ログ記録・データ保護ポリシー適用。

API: `/api/v1/ai-tools/*`

---

## 44. iPaaS 統合 — ワークフロー連携 (v0.1)

外部の iPaaS プラットフォームと Webhook トリガー経由で連携します。
...
## 45. アーティファクトエクスポート (v0.1)

タスク成果物を各種フォーマットでエクスポートし、ローカル保存または外部サービスへ送信します。
...
- ローカルファイルシステム（サンドボックスのホワイトリストフォルダ）
- Google Docs
- Notion
- n8n Webhook
...
## 46. AI 共創リパーパスエンジン (v0.1)

1 つのコンテンツを複数のメディア形式に自動変換します。ブランドボイスやスタイルガイドに基づいてトーンを調整します。

### 変換先

ブログ記事 / SNS 投稿 / ツイートスレッド / メールニュースレター / スライドデッキ / インフォグラフィック / プレスリリース / FAQ / ビデオスクリプト / ポッドキャストトランスクリプト

安全性: プロンプトインジェクション検査、PII ガード適用、監査ログ記録。

---

## 47. Obsidian 統合 (v0.1)

Obsidian Vault との双方向同期を提供します。

### 機能

| 機能 | 説明 |
|------|------|
| **ノート読み書き** | Vault 内のノート作成・更新・削除 |
| **リンクグラフ構築** | `[[wikilink]]` の自動解析とグラフ生成 |
| **全文検索** | Vault 内の全ノート検索 |
| **バックリンク解析** | 参照先→参照元の逆引き |
| **ナレッジストア統合** | ZEO Knowledge Store との双方向同期 |

ファイルアクセスはサンドボックス経由で許可されたディレクトリのみに制限。

---

## 48. クラウドサービスネイティブ統合 (v0.1)

マルチクラウド環境のリソース管理・ストレージ・サーバーレス実行・コスト見積もりを統一インターフェースで提供します。
...
すべての操作は承認ゲート経由・監査ログ記録・認証情報は secret_manager 経由。

---

## 49. スマートデバイス / VR/AR 統合 (v0.1)

IoT センサー・スマートディスプレイ・VR/AR ヘッドセット・ロボット・ドローンなどのスマートデバイスを統一的に管理します。
...
すべてのデバイス操作は承認ゲート経由・監査ログ記録・データ保護ポリシー適用。

---

## 50. ガバナンス・コンプライアンス (v0.1)

エンタープライズ向けの監査・権限管理・データポリシーを提供します。
...
## 51. Skill マーケットプレイス (v0.1)

コミュニティの Skill / Plugin / Extension を公開・検索・レビュー・インストールできるマーケットプレイスです。
...
## 52. マルチユーザー・チーム管理 (v0.1)

チームベースの認証・権限管理を提供します。

### チームロール

| ロール | 権限 |
|--------|------|
| **Owner** | 全権限（チーム設定・メンバー管理含む） |
| **Admin** | メンバー管理・AI 設定 |
| **Member** | チケット作成・タスク実行 |
| **Viewer** | 読み取り専用 |
| **AI Operator** | AI 関連リソースのみ操作可能 |

### 機能

- チーム作成・招待（メール / リンク）
- 招待の有効期限・使用制限
- リソース種別 × アクションの権限マトリクス

API: `/api/v1/teams/*`

---

## 53. Red-team セキュリティテスト (v0.1.1)

システムのセキュリティ防御を自動的にテストし、脆弱性を早期に検出・報告します。
全テストが実際のセキュリティモジュールを実ペイロードで実行します（設定検証だけではありません）。

### テストカテゴリ（8 カテゴリ、22 テスト）

| カテゴリ | テスト数 | 検証方法 |
|---------|---------|---------|
| **プロンプトインジェクション** | 4 | `prompt_guard` でペイロードをスキャンし検出を検証 |
| **データ漏洩** | 3 | `sanitizer` と `pii_guard` の両方でペイロードを実行 |
| **権限昇格** | 3 | IAM 拒否スコープ + プロンプトガード昇格検出を検証 |
| **PII 漏洩** | 3 | `pii_guard` でペイロードを実行しマスキングを検証 |
| **不正アクセス** | 2 | セキュリティヘッダーミドルウェア + 認証依存関係を検証 |
| **サンドボックス脱出** | 3 | パストラバーサル変種を含む 8 以上の危険パスをテスト |
| **レート制限バイパス** | 2 | リミッター設定 + 429 ハンドラー + ライブエンドポイントテスト |
| **認証バイパス** | 2 | algorithm=none、期限切れ、改竄された JWT トークンをテスト |

### CI 統合 (v0.1.1)

- Red-team テストはすべての PR で GitHub Actions により自動実行
- **クリティカル** な発見があればビルド失敗
- 失敗率が 20% を超えた場合に警告を発行

---

## 54. ワークスペース隔離 (v0.1)

AI エージェントのファイルアクセス・ネットワークアクセスを厳格に制限する隔離層です。

### 機能

- 初期状態でローカル・クラウド接続なし（最小権限原則）
- 内部ストレージのみアクセス可能
- チャット指示がシステム設定と異なる場合は `should_request_approval()` でユーザーに許可を求める
- 環境オーバーライドの監査ログ記録

---

## 55. Design Interview 過去失敗パターンフィードバック (v0.1)

Design Interview 実行時に、Experience Memory と Failure Taxonomy から過去の類似失敗パターンを自動検索し、「この種の目的設定は過去に○○の理由で失敗している」というフィードバックを提供します。

### 機能

| 機能 | 説明 |
|------|------|
| **目的ベース失敗検索** | ユーザーが入力した目的に類似する過去の失敗パターンを検索 |
| **頻発障害の自動警告** | 2 回以上発生した障害パターンを自動的に警告として表示 |
| **予防策の提示** | 過去の失敗に対する予防策を構造化して提示 |
| **回復成功率の提示** | 各障害パターンの回復成功率を数値で提示 |
| **Interview 質問の動的追加** | 過去の失敗パターンに基づく追加質問を自動生成 |

### フロー

```
1. ユーザーが目的を入力
2. Experience Memory を目的テキストで検索
3. Failure Taxonomy から頻発障害パターンを取得
4. マッチした失敗パターンをフィードバックとして整形
...
同一の入力を複数の LLM モデルに同時送信し、応答品質・レイテンシ・トークン数を比較できます。

### 機能概要

| 項目 | 説明 |
|------|------|
| **同時比較** | 複数モデルに同一プロンプトを送信し結果を並列取得 |
| **メトリクス記録** | モデルごとのレイテンシ・トークン数・コストを自動記録 |
| **セッション管理** | 比較セッションの履歴を保存・検索可能 |

### API エンドポイント

| エンドポイント | 説明 |
|-------------|------|
| `POST /companies/{id}/multi-model/compare` | マルチモデル比較実行 |

---

## 62. ブレインストーミングセッション (v0.1)

複数の AI エージェントによるディベート・レビュー・戦略セッションを実行できます。

### セッションタイプ

| タイプ | 説明 |
|--------|------|
| **debate** | 異なるモデル間でのディベート形式 |
| **review** | レビュー・批評セッション |
| **strategy** | 戦略立案セッション |
...
| エンドポイント | 説明 |
|-------------|------|
| `POST /companies/{id}/brainstorm` | セッション作成 |
| `POST /brainstorm/{session_id}/message` | メッセージ追加 |
| `GET /brainstorm/{session_id}` | セッション取得（会話履歴・インサイト含む） |
| `GET /companies/{id}/brainstorm/sessions` | セッション一覧 |
| `POST /brainstorm/{session_id}/status` | ステータス更新 |
| `GET /companies/{id}/brainstorm/search` | セッション検索 |

---

## 63. 会話メモリ — Conversation Memory (v0.1)

ユーザーとエージェント間の全会話を永続的に保存し、検索・統計取得が可能です。
...
| エンドポイント | 説明 |
|-------------|------|
| `POST /companies/{id}/conversation-memory` | メッセージ保存 |
| `GET /companies/{id}/conversation-memory` | 履歴取得（フィルタリング） |
| `GET /companies/{id}/conversation-memory/search` | 全文検索 |
| `GET /companies/{id}/conversation-memory/stats` | 会話統計 |

---

## 64. テキスト解析サービス (v0.1)

Unicode 文字を詳細にカテゴリ分類し、正確な文字数カウントを提供します。
...
| エンドポイント | 説明 |
|-------------|------|
| `POST /text/analyze` | テキスト解析実行 |

---

## 65. ロールベースモデル設定 (v0.1)

エージェントの役割（ロール）ごとに使用する LLM モデルを指定でき、フォールバックモデルも設定可能です。

### API エンドポイント

| エンドポイント | 説明 |
|-------------|------|
| `PUT /companies/{id}/role-models` | ロール別モデル設定 |
| `GET /companies/{id}/role-models` | 全ロール設定取得 |
| `GET /companies/{id}/role-models/{role}` | 特定ロール設定取得 |
| `DELETE /role-models/{config_id}` | 設定削除 |

---

## 66. AI 組織動的管理 (v0.1)

ランタイムでエージェントをロール単位で追加・削除・更新できます。カスタムシステムプロンプトにも対応します。

### API エンドポイント

| エンドポイント | 説明 |
|-------------|------|
| `POST /companies/{id}/agents/by-role` | ロール指定でエージェント追加 |
| `DELETE /agents/{id}/remove` | エージェント削除 |
| `PATCH /agents/{id}/role` | エージェントロール更新 |

---

## 67. カスタムエージェントロール (v0.1)

プリセットロールに加え、ユーザー独自のカスタムロールを定義できます。

### API エンドポイント

| エンドポイント | 説明 |
|-------------|------|
| `POST /companies/{id}/custom-roles` | カスタムロール作成 |
| `GET /companies/{id}/custom-roles` | カスタムロール一覧 |
| `DELETE /custom-roles/{role_id}` | カスタムロール削除 |
| `GET /companies/{id}/available-roles` | 利用可能全ロール（プリセット＋カスタム） |

---

## 68. 自然言語フィーチャーリクエスト (v0.1)

自然言語で AI 組織への要望を送信でき、意図を自動解釈して実行提案または自動実行します。

### API エンドポイント

| エンドポイント | 説明 |
|-------------|------|
| `POST /companies/{id}/feature-requests` | フィーチャーリクエスト送信 |
| `GET /companies/{id}/feature-requests` | リクエスト一覧 |

---

## 69. ブレインダンプ / Secretary AI (v0.1)

CEO・ユーザーが思いつきやメモを自由にダンプし、自動分類・タグ付け・アクションアイテム抽出を行います。
...
| エンドポイント | 説明 |
|-------------|------|
| `POST /companies/{id}/brain-dump` | ブレインダンプ作成 |
| `GET /companies/{id}/brain-dumps` | 一覧（フィルタリング） |
| `GET /brain-dumps/{id}` | 詳細取得 |
| `PATCH /brain-dumps/{id}` | 更新 |
| `GET /companies/{id}/brain-dumps/search` | 検索 |
| `GET /companies/{id}/brain-dumps/action-items` | 全アクションアイテム抽出 |
| `GET /companies/{id}/brain-dumps/daily-stats` | 日次統計 |

---

## 70. ファイルアップロード管理 (v0.1)

最大 50MB のファイルをアップロードし、拡張子ホワイトリストで安全に管理します。

### 対応フォーマット

| カテゴリ | 拡張子 |
|---------|--------|
| **ドキュメント** | txt, md, csv, json, yaml, xml, html, pdf, docx, xlsx, pptx |
| **画像** | png, jpg, jpeg, gif, svg, webp |
| **コード** | py, js, ts, tsx, jsx, rb, go, rs, java, c, cpp, h |
| **アーカイブ** | zip, tar, gz |
...
| エンドポイント | 説明 |
|-------------|------|
| `POST /files/upload` | 単一ファイルアップロード |
| `POST /files/upload-multiple` | 複数ファイルアップロード |
| `GET /files/{id}` | ファイル情報取得 |
| `GET /files/{id}/download` | ファイルダウンロード |
| `DELETE /files/{id}` | ファイル削除 |
| `GET /files` | ファイル一覧（ページネーション） |

---

## 71. タスク実行中のユーザー入力 (v0.1)

タスク実行中に AI がユーザーへ追加情報を要求でき、タイムアウト付きで応答を待機します。
...
| エンドポイント | 説明 |
|-------------|------|
| `POST /user-input/request` | 入力リクエスト作成 |
| `GET /user-input/pending` | 全保留リクエスト取得 |
| `GET /user-input/pending/{task_id}` | タスク別保留リクエスト |
| `POST /user-input/{id}/answer` | 回答送信 |
| `DELETE /user-input/{id}` | リクエストキャンセル |

---

## 72. リソースインポート (v0.1)

業務マニュアル・規定・ドキュメントをファイル・フォルダ・URL からインポートし、AI のコンテキストとして活用します。
...
| エンドポイント | 説明 |
|-------------|------|
| `POST /resources/import/file` | ファイルインポート |
| `POST /resources/import/folder` | フォルダインポート（再帰） |
| `POST /resources/import/url` | URL からインポート |
| `GET /resources` | リソース一覧（フィルタリング） |
| `GET /resources/search` | 全文検索 |
| `GET /resources/{id}` | リソース詳細 |
| `DELETE /resources/{id}` | リソース削除 |

---

## 73. プロジェクト・ゴール管理 (v0.1)

プロジェクトを作成し、階層的なゴール（目標）を定義できます。

### API エンドポイント

| エンドポイント | 説明 |
|-------------|------|
| `GET /companies/{id}/projects` | プロジェクト一覧 |
| `POST /companies/{id}/projects` | プロジェクト作成 |
| `GET /projects/{id}/goals` | ゴール一覧 |
| `POST /projects/{id}/goals` | ゴール作成 |

---

## 74. 仮説エンジン — Hypothesis Engine (v0.1)

複数エージェントによる仮説の並列検証・クロスレビューを行います。
...
## 75. AI 調査エンジン — AI Investigator (v0.1)

AI がデータベースに対して安全な読み取り専用クエリを実行し、トラブルシューティングや分析を行います。
...
|------|------|
| **読み取り専用** | SELECT のみ許可 |
| **テーブルホワイトリスト** | tickets, tasks, agents, audit_logs, knowledge_store 等 |
| **SQL インジェクション防御** | 禁止キーワードの検出 |
| **行数制限** | 1 クエリあたり最大 500 行 |
...
## 76. ブラウザ自動操作 — Browser Automation (v0.1)

Playwright ベースの Web ブラウザ自動操作で、承認ゲートと監査ログ付きで安全に実行します。
...
- 承認ゲート（`approval_gate.py`）による実行前承認
- 全操作の監査ログ記録
- データ保護ポリシーの適用

---

## 77. LSP 統合 — Language Server Protocol (v0.1)

Language Server Protocol を使用したコード補完・ホバー・定義ジャンプ・リファレンス検索・診断を提供します。
...
| `definition` | 定義ジャンプ |
| `references` | リファレンス検索 |
| `diagnostics` | 診断（エラー・警告） |
| `formatting` | コードフォーマット |
| `rename` | リネーム |

---

## 78. MCP 統合 — Model Context Protocol (v0.1)

Model Context Protocol (MCP) を介して外部ツール・リソース・プロンプトを統合します。

### API エンドポイント

| エンドポイント | 説明 |
|-------------|------|
| `GET /mcp/capabilities` | MCP サーバー能力取得 |
| `GET /mcp/tools` | 利用可能ツール一覧 |
| `POST /mcp/tools/call` | ツール呼び出し |
| `GET /mcp/resources` | リソース一覧 |
| `GET /mcp/prompts` | プロンプト一覧 |

---

## 79. 外部スキル検索・インポート (v0.1)

GitHub・各種スキルレジストリからスキルを検索し、ワンクリックでインストールできます。

### 対応ソース

| ソース | 説明 |
|--------|------|
| `github_agent_skills` | GitHub Agent Skills リポジトリ |
| `skills_sh` | skills.sh レジストリ |
| `openclaw` | OpenClaw レジストリ |
| `claude_code` | Claude Code スキル |
| `git_repo` | 任意の Git リポジトリ |
| `url` | URL 直接指定 |
...
| エンドポイント | 説明 |
|-------------|------|
| `POST /skills/external/search` | 外部スキル検索 |
| `POST /skills/external/import` | スキルインポート・インストール |

---

## 80. 入力セキュリティミドルウェア (v0.1)

全 API エンドポイントの入力に対して、プロンプトインジェクション検査・PII 検出を自動適用するミドルウェアです。

### 検査内容

| 検査 | 説明 |
|------|------|
| **プロンプトインジェクション** | 28+ パターンによる 5 段階脅威検出 |
| **PII 検出** | 13 カテゴリの個人情報検出・マスキング |
| **外部データラッピング** | LLM に渡す外部データの境界マーカー付与 |

### 実装

- `InputSanitizationMiddleware` — リクエストボディのスキャン
- 脅威レベル CRITICAL/HIGH 検出時はリクエストを拒否（HTTP 422）
- PII 検出はログ記録のみ（マスキングはサービス層で実施）

---

## 81. 多言語対応 (i18n) (v0.1)

日本語・英語・中国語・韓国語・ポルトガル語・トルコ語の 6 言語に対応。UI 表示、AI への指示、CLI 出力すべてが選択言語に切り替わります。
...
| 方法 | 操作 |
|------|------|
| **セットアップウィザード** | 初回起動時に言語を選択（最初のステップ） |
| **設定画面 (UI)** | 設定 → 言語設定 → インターフェース言語を切替 |
| **Config API** | `PUT /api/v1/config { "key": "LANGUAGE", "value": "en" }` |
| **自然言語コマンド** | 「言語を英語に変更して」/ "Change language to English" |
| **CLI フラグ** | `zero-employee local --lang en` / `zero-employee chat --lang zh` |
| **環境変数** | `LANGUAGE=en zero-employee serve` |

### 翻訳対象

| レイヤー | 仕組み | ファイル |
|---------|--------|---------|
| バックエンド CLI/TUI | Python 辞書ベース翻訳（80+ キー） | `app/core/i18n.py` |
| フロントエンド UI | Zustand + JSON ロケール（320+ キー） | `shared/i18n/locales/{ja,en,zh,ko,pt,tr}.json` |
| AI システムプロンプト | 言語別のプロンプトテンプレート | `app/cli.py` |
| ブラウザアシスト | 言語別の画面分析指示 | `integrations/browser_assist.py` |

### 設定の永続化

| コンポーネント | 保存先 |
|---------------|--------|
| バックエンド | `~/.zero-employee/config.json` |
| フロントエンド | ブラウザ `localStorage` |
| 環境変数 | `LANGUAGE` 環境変数（最優先） |

### デスクトップインストーラー

Windows インストーラーは単一の `.exe` セットアップファイルを提供。アプリ内のセットアップウィザードで言語を選択するため、言語別インストーラーは不要です。
