...
# → 在 http://localhost:5173 開啟 Web UI
```
...
| **競品同等原語** | 工作流程範本（Dify 風格）、Crew（CrewAI 風格）、DAG 節點結果快取（LangGraph 風格）、通用 HTTP 連接器（n8n 風格）、Microsoft Graph 轉接器（M365） — 全部可在桌面 UI 中一鍵呼叫 |
