...
| | ChatGPT 等 | RPA | n8n / Make | **ZEO** |
|---|---|---|---|---|
| 入力方法 | テキスト | フロー設計 | ノード配置 | **自然言語** |
| AI の数 | 1対1 | なし | API 呼出 | **複数 AI チーム** |
| 記憶 | 会話で消失 | なし | なし | **永続 Experience Memory** |
| 品質保証 | 目視確認 | ルール | なし | **Judge Layer 自動検証** |
| 失敗対応 | 手動やり直し | 停止 | 停止 | **Self-Healing 自動再計画** |
| 承認フロー | なし | 手動設定 | 手動設定 | **自動検出・強制ブロック** |
| 監査ログ | なし | 限定的 | 限定的 | **全操作記録** |
...
| SettingsPage | 実装済み | LLM 設定・Ollama・i18n |
...
詳細は [docs/FEATURE_BOUNDARY.md](FEATURE_BOUNDARY.md) を参照。
...
| `ABOUT.md` | なぜ ZEO が必要か・従来ツールとの比較 | 非エンジニア・経営者 |
| `USER_GUIDE.md` | セットアップから操作方法まで | エンドユーザー |
| **`docs/OVERVIEW.md`（本書）** | **思想・機能・構造の総合解説** | **初見の方** |
| `docs/FEATURE_BOUNDARY.md` | コア vs 拡張の機能境界定義 | 開発者 |
| `Zero-Employee Orchestrator.md` | 最上位基準文書（思想・要件） | 設計者 |
| `DESIGN.md` | 実装設計書（DB・API・状態遷移） | 実装者 |
| `MASTER_GUIDE.md` | 実装運用ガイド | AI エージェント・実装者 |
| `CLAUDE.md` | Claude Code 向け開発ガイド | AI エージェント |
| `SECURITY.md` | セキュリティポリシー | 運用者 |
| `CHANGELOG.md` | 変更履歴 | 全員 |
| `instructions_section2〜7` | 各領域の実装指示 | 実装者 |
...
│   │   │   ├── api/routes/     # REST API (16 ルート)
│   │   │   ├── api/ws/         # WebSocket
│   │   │   ├── models/         # ORM モデル (21 テーブル)
│   │   │   ├── schemas/        # Pydantic DTO
│   │   │   ├── services/       # ビジネスロジック
│   │   │   ├── repositories/   # DB 入出力抽象化
│   │   │   ├── orchestration/  # DAG・Judge・状態機械・Memory
│   │   │   ├── heartbeat/      # 定期実行スケジューラ
│   │   │   ├── providers/      # LLM Gateway・Ollama・g4f・RAG
│   │   │   ├── tools/          # 外部ツール接続
│   │   │   ├── policies/       # 承認ゲート・自律実行境界
│   │   │   ├── security/       # Secret Manager・Sanitizer
│   │   │   ├── audit/          # 監査ログ
│   │   │   └── tests/          # テスト (11 ファイル)
│   │   └── alembic/            # DB マイグレーション
│   ├── desktop/                # Tauri デスクトップアプリ
│   │   └── ui/src/             # React フロントエンド (19 画面)
