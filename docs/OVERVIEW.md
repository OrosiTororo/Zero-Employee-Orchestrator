...
### API エンドポイント（実装済み・24 ルートモジュール）

| エンドポイント群 | 主要機能 |
|-----------------|---------|
| `/auth` | ログイン・登録・匿名セッション・OAuth |
| `/companies` | 会社 CRUD・ダッシュボード |
| `/tickets` | チケット作成・一覧・詳細・状態遷移・ファイル添付 |
| `/specs_plans` | Spec / Plan の作成・承認 |
| `/tasks` | タスク作成・実行・完了 |
| `/agents` | エージェント管理・役割別追加・一時停止・再開 |
| `/approvals` | 承認一覧・承認・却下 |
| `/artifacts` | 成果物管理 |
| `/audit` | 監査ログ一覧・フィルタ |
| `/budgets` | 予算ポリシー・コスト管理 |
| `/heartbeats` | 定期実行ポリシー・実行履歴 |
| `/registry` | Skill / Plugin / Extension 検索・CRUD・自然言語生成 |
| `/ollama` | ローカル LLM 直接操作・RAG |
| `/settings` | アプリ設定・ツール接続 |
| `/config` | ランタイム設定管理 |
| `/models` | モデルカタログ・ヘルスチェック・廃止管理 |
| `/observability` | 推論トレース・通信ログ・実行監視 |
| `/self-improvement` | AI 自己改善（Skill 分析・改善・Judge 調整・A/B テスト） |
| `/multi-model` | マルチモデル比較・壁打ち・会話記憶・役割別設定 |
| `/secretary` | 秘書AI（ブレインダンプ・日次サマリー） |
| `/knowledge` | ナレッジストア・変更検知 |
| `/platform` | MCP・Sentry・IAM・仮説検証・セッション・調査 |
| `/projects` | プロジェクト・ゴール管理 |
...
| `test_chaos_dag.py` | カオステスト（Self-Healing DAG） |
| `test_ollama_integration.py` | Ollama 統合 |
| `zeo_bench.py` | ZEO-Bench（Judge Layer 200問ベンチマーク） |
...
→ `plugins/` にマニフェスト定義済み（9 Plugin）。業務特化ロジックは本体に入れない
...
| `docs/FEATURES.md` | 実装済み機能の全体一覧（34セクション） | 機能確認・評価者 |
...
| `docs/PROPOSAL.md` | プロジェクト提案書（未踏形式） | 助成金審査員・スポンサー |
| `docs/TITLE_PROPOSALS.md` | タイトル案10選と申請概要 | 助成金審査員・スポンサー |
| `docs/SCALING_AND_COSTS.md` | コスト・制約・大規模活用ガイド | 導入検討者・運用者 |
...
│   ├── research/               # リサーチ
│   ├── backoffice/             # バックオフィス
│   └── ai-self-improvement/    # AI 自己改善（Skill 分析・改善提案・A/B テスト）
