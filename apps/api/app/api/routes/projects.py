...
async def list_projects(company_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    """プロジェクト一覧"""
...
async def create_project(company_id: str, req: ProjectCreate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    """プロジェクト作成"""
    project = Project(
        id=uuid.uuid4(),
...
async def list_goals(project_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    """プロジェクトの目標一覧"""
...
async def create_goal(project_id: str, req: GoalCreate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    """目標作成"""
    goal = Goal(
        id=uuid.uuid4(),
