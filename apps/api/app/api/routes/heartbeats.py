...
async def list_policies(company_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return [
        {
            "id": str(p.id), "name": p.name, "cron_expr": p.cron_expr,
...
async def create_policy(company_id: str, req: HeartbeatPolicyCreate, db: AsyncSession = Depends(get_db)):
    """Heartbeatポリシー作成"""
    policy = HeartbeatPolicy(
        id=uuid.uuid4(), company_id=uuid.UUID(company_id),
        name=req.name, cron_expr=req.cron_expr, timezone=req.timezone,
        enabled=req.enabled, jitter_seconds=req.jitter_seconds,
...
async def list_runs(company_id: str, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(HeartbeatRun).where(HeartbeatRun.company_id == cid).order_by(HeartbeatRun.started_at.desc()).limit(50)
    )
