...
async def list_budgets(company_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return [
        {
            "id": str(b.id), "name": b.name, "scope_type": b.scope_type,
            "period_type": b.period_type, "limit_usd": float(b.limit_usd),
...
async def create_budget(company_id: str, req: BudgetPolicyCreate, db: AsyncSession = Depends(get_db)):
    """予算ポリシー作成"""
    policy = BudgetPolicy(
        id=uuid.uuid4(), company_id=uuid.UUID(company_id),
        name=req.name, scope_type=req.scope_type,
        period_type=req.period_type, limit_usd=req.limit_usd,
