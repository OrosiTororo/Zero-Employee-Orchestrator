...
export default function BrainstormPage() {
  ...
  async function createSession() {
    try {
      ...
      setNewTitle("")
      setNewTopic("")
    } catch { addToast((t as unknown as Record<string, Record<string, string>>).errors?.brainstormSessionFailed ?? "Failed to create session") }
  }
  ...
  async function sendMessage() {
    ...
    try {
      ...
      analyzeText(input)
    } catch { addToast((t as unknown as Record<string, Record<string, string>>).errors?.brainstormMessageFailed ?? "Failed to send message") }
  ...
  async function runMultiModelComparison() {
    ...
    } catch { addToast((t as unknown as Record<string, Record<string, string>>).errors?.brainstormCompareFailed ?? "Failed to run comparison") }
  ...
  async function submitFeatureRequest() {
    ...
    try {
      ...
      setFeatureRequest("")
      loadAvailableRoles()
    } catch { addToast((t as unknown as Record<string, Record<string, string>>).errors?.brainstormRequestFailed ?? "Failed to submit request") }
  }

  async function addAgentByRole(role: string) {
    try { await api.post(`/companies/${companyId}/agents/by-role`, { role }) }
    catch { addToast((t as unknown as Record<string, Record<string, string>>).errors?.brainstormAgentFailed ?? "Failed to add agent") }
  }
